...
function renderMiniAppStage(id) {
  ...
  iframe.addEventListener('load', () => {
    if (stageRenderToken !== token || !iframe.isConnected) return;
...
function disposeStageChildren(host) {
  ...
  children.forEach(child => {
    if (child instanceof HTMLIFrameElement) {
      try {
        ...
        console.warn('mini-app stage: unable to reset iframe', error);
      }
    }
    child.remove();
  });
